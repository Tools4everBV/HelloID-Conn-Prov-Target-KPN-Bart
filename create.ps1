...
# unused attribues:
        ...
        #EmailAliasesToAdd           = @{}                      # Lievegoed heeft geen aliassen
        #Info                       = ""                        # Niet gebruiken
...
$account = @{
        ...
        Password                    = (ConvertTo-SecureString (New-RandomPassword($config.passwordLength)) -Force -AsPlainText)
...
if (-not [string]::IsNullOrEmpty($aRef)) {
    ...
    # Correlate user
    Write-Verbose -Verbose "Existing account found. Correlating person. (Using attribute '$correlationAccountField' and value '$correlationPersonField'. ObjectGuid: '$aRef')"
    ...
    try {
        ...
        throw("Name generation failed with error: $($_.Exception.Message)")
    }
    ...
    if (-not ($dryRun -eq $true)) {
        try {
            ...
            if ($createResult.Error -eq $true) {
                ...
                throw ( $createResult | ConvertTo-Json) # werkt dit?
            }        
        } catch {
    ...
    if ($account.contains("isActive") -and $account.IsActive -eq $false ) {
        Write-Verbose -Verbose "User creation: Disabling user"
        if (-not ($dryRun -eq $true)) {
            try {
...
foreach ($keyValue in $accountUpdate.GetEnumerator()) {
    ...
    Write-Verbose -Verbose "Update: setting attribute '$($keyValue.Key)' to value '$($keyValue.Value)'"
}
...
# Set Manager (TODO: should calculate if this is required, check previousAccount) -- this will now leave an incorrect manager in when a new manager is not available. This might be correct for creating, but this is not OK when updating.
# Can only do this when the connector is live and mRef is filled in.
if (-not [string]::IsNullOrEmpty($account.managerObjectGuid)) {
    $managerIdentity = [KPNBartConnectedServices.CommandService.ObjectIdentity]::new()
    $managerIdentity.IdentityType = "Guid"
    $managerIdentity.Value = $account.managerObjectGuid
    Write-verbose -verbose "User creation/update: Setting manager"
    if (-not ($dryRun -eq $true)) {
        try {
